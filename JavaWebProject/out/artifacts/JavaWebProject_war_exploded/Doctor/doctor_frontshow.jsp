...
<div class="container">
	...
	<div class="row bottom15">
		...
		<div class="col-md-6 col-xs-6">
			<button onclick="doctorBook('<%=doctor.getDoctorNumber() %>','<%=(String)session.getAttribute("user_name")%>')" class="btn btn-info">预约此医生</button>
...
/*预约医生*/
function doctorBook(doctorNumber,userName) {
	//判断用户是否登录状态 如果未登录需要登录后才能预约
	if(userName == 'null' || userName == undefined || userName == ''){
		alert("必须登录后才能预约");
		return;
	}
	//开始预约 携带医生的doctorNumber 去到添加预约信息页面

	location.href = '<%=basePath %>OrderInfo/orderInfo_frontAdd.jsp?doctorNumber='+doctorNumber;
}
 </script> 
</body>
</html>
