		...
		<div class="col-md-12 wow fadeInUp" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	<li role="presentation" class="active"><a href="<%=basePath %>OrderInfo/frontlist">预约信息列表</a></li>
				...
				<div class="tab-content">
				    ...
				    <div role="tabpanel" class="tab-pane active" id="orderInfoAdd"> 
						  ...
						  <div class="form-group" style="display: none">
						  	 ...
						  	 <div class="col-md-8">
							    <input type="text" id="orderInfo_checkState" name="orderInfo.checkState" class="form-control" value="无" placeholder="请输入处理状态">
							 </div>
						  </div>
						  <div class="form-group" style="display: none">
						  	 <label for="orderInfo_replyContent" class="col-md-2 text-right">医生回复:</label>
						  	 <div class="col-md-8">
							    <textarea id="orderInfo_replyContent" name="orderInfo.replyContent" rows="8" class="form-control" placeholder="请输入医生回复">暂无回复</textarea>
							 </div>
						  </div>
				          <div class="form-group">
...
$(function(){
	...
	$.ajax({
		...
		success: function(userInfos,response,status) {
    		...
    		$(userInfos).each(function(i,userInfo){
    			if(userInfo.user_name == "<%=session.getAttribute("user_name") %>"){
	...
	$.ajax({
		...
		success: function(doctors,response,status) { 
			...
			var html="";
			//获取对应的医生的doctorNumber值
			var doctorNumber = '${param.doctorNumber}';
			if(doctorNumber == ''){
				...
				//doctorNumber不为空
				$(doctors).each(function(i,doctor){
					if(doctorNumber != '' && doctorNumber == doctor.doctorNumber) {
						html = "<option value='" + doctor.doctorNumber + "'>" + doctor.doctorName + "</option>";
						$("#orderInfo_doctorObj_doctorNumber").html(html);
						return;
					}
